...
# The second argument is the mode in which the file should be opened. 
# In this example, we are opening the file for reading, so we use the 
# 'r' mode.
...
# Next, in this particular case, it would be helpful to break this 
# string up into a list. We can do that by using the split() method.
...
# The with statement is a way to open a file and automatically close 
# it when we're done with it. It is generally a good idea to use the 
# with statement when working with files because it ensures that the 
